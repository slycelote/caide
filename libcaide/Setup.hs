...
inlinerConfHook (pkg, pbi) flags = do
    ...
    lbi' <- if not cppinliner
        then return lbi
            ...
            rawSystemExit verbosity "cmake" $ ["--build", inlinerBuildAbsDir, "--target", "caideInliner"] ++ cmakeBuildArgs

            -- Read the list of dependencies generated by CMake at configure stage.
            clangLibs <- split ';' <$> readFile (inlinerBuildAbsDir </> "caide-libs.txt")

            let addInlinerLibs bi = bi {
                    extraLibs = ["caideInliner"] ++ clangLibs ++ extraLibs bi,
                    extraLibDirs = [inlinerBuildAbsDir, inlinerBuildAbsDir </> "llvm-project" </> "llvm" </> "lib"] ++ extraLibDirs bi
                }
