...
typedef std::map<Stmt*, Stmt*> MapTy;
...
StmtParentMap::~StmtParentMap() {
  delete (MapTy*) Impl;
}

bool StmtParentMap::contains(clang::Stmt* S) const {
  const MapTy *M = reinterpret_cast<const MapTy *>(Impl);
  return M->find(S) != M->end();
}

void StmtParentMap::addStmt(Stmt* S) {
  if (S) {
...
void StmtParentMap::setParent(const Stmt *S, const Stmt *Parent) {
  assert(S);
  assert(Parent);
  MapTy *M = reinterpret_cast<MapTy *>(Impl);
...
Stmt* StmtParentMap::getParent(Stmt* S) const {
  MapTy* M = (MapTy*) Impl;
  MapTy::iterator I = M->find(S);
  return I == M->end() ? nullptr : I->second;
}

Stmt *StmtParentMap::getParentIgnoreParens(Stmt *S) const {
  do { S = getParent(S); } while (S && isa<ParenExpr>(S));
  return S;
}

Stmt *StmtParentMap::getParentIgnoreParenCasts(Stmt *S) const {
  do {
    S = getParent(S);
  }
...
Stmt *StmtParentMap::getParentIgnoreParenImpCasts(Stmt *S) const {
  do {
    S = getParent(S);
...
Stmt *StmtParentMap::getOuterParenParent(Stmt *S) const {
  Stmt *Paren = nullptr;
...
bool StmtParentMap::isConsumedExpr(Expr* E) const {
  Stmt *P = getParent(E);
  Stmt *DirectChild = E;
